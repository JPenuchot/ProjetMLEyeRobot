/*
 * Implementation of the IO interface for the JPEG format
 * 
...
public class JPEG implements IO {

	/**
	 * Read an image from a JPEG
	 * 
	 * @param src
	 ...
	 * All *.jpg|*.jpeg files will be added to the database
	 * 
	 * @param src
	...
	public ImageDB readDB(String src) throws IOException {
		...
		for (File f : list) {
			if (f.isFile() && (f.getName().substring(f.getName().length() - 4).equals(".jpg") ||  f.getName().substring(f.getName().length() - 5).equals(".jpeg"))) {
	...
	public void write(Image src, String dest) {
		...
		// Save the file
		try {
			ImageIO.write(biOut, "JPEG", new File(dest));
		} catch (IOException e) {
			e.printStackTrace();
		}
	 ...
	 * File names will be like 1.jpg, 2.jpg, 3.jpg, etc.
	 * 
	 * @param src
	...
	public void writeDB(ImageDB src, String dest) {
		...
		int current = 0;
		for (Image img : src) {
			write(img, dest + addSlash + current + ".jpg");
			current++;
		}
